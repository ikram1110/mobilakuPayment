<?php
  ...
  $response_json = (array)json_decode($response_raw);
  if($response_json['status'] === "000" ){
    $response = bni_hash('decrypt',$response_json,$client_id,$secret_key);
    echo json_encode(["status" => $response_json["status"], "data" => $response]);
  } else {
    echo json_encode(["status" => $response_json['status'],"message" => $response_json['message']]);
  }
  ...
  function bni_hash($type,$data,$client,$key){
    ...
    if($type == "decrypt"){
      return BniEnc::decrypt($data['data'],$client,$key);
    }
  }
